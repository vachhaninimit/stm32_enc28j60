...
extern  uint8_t myPort;
extern  uint8_t myMac[];
extern  uint8_t myIpa[];
...
unsigned  int checksum(unsigned char *buf, unsigned  int len,unsigned char type)
	...
	while(len >1)
		{
			sum += 0xFFFF & (*buf<<8|*(buf+1)); // 1 << 8 = 2^8 ,,, 1 char = 1 byte or 0x45 = 1byte = 1 char.
			buf+=2;
			len-=2;
		}
...
// you must call this function once before you use any of the other functions:
	//Gan gia IP, MAC address & port number cho bien static
...
unsigned char eth_type_is_arp_and_my_ip(unsigned char *buf,unsigned  int len)
	...
	while(i<4)
		{
	    if(buf[ETH_ARP_DST_IP_P+i] != myIpa[i])
			{
	        return(0);
	    	}
	    i++;
...
void make_ip(unsigned char *buf)
	...
	while(i<4)
        ...
        buf[IP_SRC_P+i]=myIpa[i]; //ipaddr la ip cua STM32
        i++;
		}
	...
	void make_ip_EDIT(unsigned char *buf)
	{
 	unsigned char ipaddr_PC[4] = {192,168,1,95}; //dia chi tinh cua PC ket noi voi STM32
	unsigned char i=0;
	while(i<4)
		{
        buf[IP_DST_P+i]=ipaddr_PC[i]; //
        buf[IP_SRC_P+i]=myIpa [i];//ipaddr la ip cua STM32
        i++;
		}
	fill_ip_hdr_checksum(buf);
...
void make_tcphead(unsigned char *buf,unsigned  int rel_ack_num,unsigned char mss,unsigned char cp_seq)
	{
	unsigned char port_PC[2] = {0,100}; //dia chi tinh cua PC ket noi voi STM32
	unsigned char i=0;
	unsigned char tseq;
	while(i<2)
		{
	    buf[TCP_DST_PORT_H_P+i]=buf[TCP_SRC_PORT_H_P+i];//org
			//buf[TCP_DST_PORT_H_P+i]=0; //edit
	    buf[TCP_SRC_PORT_H_P+i]=0; // clear source port
	    i++;
		}
	// set source port  (http):
	buf[TCP_SRC_PORT_L_P]=myPort; 
	i=4;
	// sequence numbers:
...
void make_arp_answer_from_request(unsigned char *buf)
	...
	while(i<4)
        ...
        buf[ETH_ARP_SRC_IP_P+i]=myIpa[i];
        i++;
		}
	// eth+arp is 42 bytes:
...
void make_udp_reply_from_request(unsigned char *buf,char *data,unsigned char datalen,unsigned  int port)
	{
		...
		buf[IP_TOTLEN_H_P]=j>>8; //0
		buf[IP_TOTLEN_L_P]=j & 0xff ;//IP_HEADER_LEN+UDP_HEADER_LEN+datalen;
		//make_ip(buf);
	  make_ip_EDIT(buf);
		buf[UDP_DST_PORT_H_P]=port>>8;
		buf[UDP_DST_PORT_L_P]=port & 0xff;
		// sent to port of sender and use "port" as own source:
		buf[UDP_DST_PORT_H_P]=0;//buf[UDP_SRC_PORT_H_P];
		buf[UDP_DST_PORT_L_P]= port;//buf[UDP_SRC_PORT_L_P];
...
void send_udp_data(unsigned char *buf, char *data, unsigned char datalen, unsigned  int port)
	{
		unsigned char i =0;
		unsigned  int j =0;  
	  unsigned char ipaddr_PC[4] = {192,168,1,95}; //dia chi tinh cua PC ket noi voi STM32
	  unsigned char ipaddr_ST[4] = {192,168,1,25}; //dia chi tinh  STM32
		
	  unsigned char macaddr_PC[6] = {0xb8,0x2a,0x72,0xf1,0xb8,0xd8}; //dia chi tinh cua PC ket noi voi STM32
	  unsigned char macaddr_ST[6] = {0x54,0x55,0x58,0x10,0x00,0x24}; //dia chi tinh  STM32 
		//
	  buf[IP_PROTO_P]= IP_PROTO_UDP_V; 
		...
		while(i<6)
			{
					buf[ETH_DST_MAC +i]=macaddr_PC[i];
					buf[ETH_SRC_MAC +i]=macaddr_ST[i]; 
					i++;
			}
		if (datalen>220)
			{
	  ...
	  //make_ip_EDIT(buf);
		...
		{
        buf[IP_DST_P+i]=ipaddr_PC[i]; //
        buf[IP_SRC_P+i]=ipaddr_ST[i];//ipaddr la ip cua STM32
        i++;
		}
	  fill_ip_hdr_checksum(buf);
		//------------------------Make IP 
	...
	void TEST(unsigned char *buf, char *data, unsigned char datalen, unsigned  int port)
	{
		unsigned char i=0;
		unsigned  int j ;  
	  unsigned char ipaddr_PC[4] = {192,168,1,95}; //dia chi tinh cua PC ket noi voi STM32
...
unsigned  int fill_tcp_data_len(unsigned char *buf, unsigned  int  pos, const unsigned char *s, char len)
{
        // fill in tcp data at position pos
        //
        // with no options the data starts after the checksum + 2 more bytes (urgent ptr)
...
void make_tcp_ack_with_data(unsigned char *buf,unsigned  int dlen)
	{
		...
		// the fin here:
		buf[TCP_FLAGS_P]=TCP_FLAGS_ACK_V|TCP_FLAGS_PUSH_V;//|TCP_FLAGS_FIN_V; // TCP_FLAGS_FIN_V cause the Close Connection after sending.
