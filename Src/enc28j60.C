...
unsigned char	SPI1_ReadWrite(unsigned char Byte);
...
void enc28j60Init(SPI_HandleTypeDef *spi, unsigned char* macaddr)
	{
		hspi = spi;
	// initialize I/O
	//enc28j60CSinit();
...
unsigned char	SPI1_ReadWrite(unsigned char Byte)
{
	...
	if(HAL_SPI_TransmitReceive(hspi,(uint8_t*) &Byte,(uint8_t*) &receivedbyte,1,0x2000)!=HAL_OK){
